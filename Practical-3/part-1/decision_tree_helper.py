...
def divide_data(data, feature_column, feature_val):
    ...
    if type(feature_val) == int or type(feature_val) == float:
        data1 = [row for row in data if row[feature_column] >= feature_val]
        data2 = [row for row in data if row[feature_column] < feature_val]
...
def gini_impurity(data1, data2):
    ...
    if N1 == 0 and N2 == 0:
        return 0
    if N1 == 0:
        y2_counts = np.array([float(len(list(group))) for key, group in groupby(get_columns(data2, len(data2[0]) - 1))])
        return N2-np.dot(y2_counts, y2_counts)/N2
    if N2 == 0:
        y1_counts = np.array([float(len(list(group))) for key, group in groupby(get_columns(data1, len(data1[0]) - 1))])
        return N1-np.dot(y1_counts, y1_counts)/N1
    y1_counts = np.array([float(len(list(group))) for key, group in groupby(get_columns(data1, len(data1[0]) - 1))])
    y2_counts = np.array([float(len(list(group))) for key, group in groupby(get_columns(data2, len(data2[0]) - 1))])
    return N1 - np.dot(y1_counts, y1_counts)/N1 + N2-np.dot(y2_counts, y2_counts)/N2
...
def build_tree(data, current_depth=0, max_depth=1e10):
    ...
    dict_current_results = dict_of_values(data)
    result = max(dict_current_results, key=lambda key: dict_current_results[key])
    if (current_depth == max_depth):
        return DecisionNode(current_results=dict_current_results, result=result, is_leaf=True)
    ...
    return DecisionNode(true_branch=t1, false_branch=t2,
                        current_results=dict_current_results, result=result, column=best_column, value=best_value)
